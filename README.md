...
Pinned release: `@v0.7.1`. Use a git tag for reproducible installs; use `@main` only if you accept moving-head changes.

See [CHANGELOG.md](CHANGELOG.md) and [docs/releases/v0.7.1.md](docs/releases/v0.7.1.md) for idempotent capability promote. Capability registry writing: [v0.7.0](docs/releases/v0.7.0.md).
...
pipx install "openclaw-governance @ git+https://github.com/pawlsclick/openclaw-governance@v0.7.1"
...
```bash
pip install "openclaw-governance @ git+https://github.com/pawlsclick/openclaw-governance@v0.7.1"
```
...
After `init`, commit the governance root and enable `.github/workflows/governance-drift.yml`. It installs `@v0.7.1` from git, runs `openclaw-gov regen --check` and `openclaw-gov check`, and runs `openclaw-gov discover --staged` with `git diff --exit-code workflows/registry.yaml` so staged discovery cannot mutate the registry in CI.
